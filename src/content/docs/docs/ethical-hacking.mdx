...
// SSR: obtiene la fecha de última modificación real usando el endpoint API
export let lastModified = "";
export async function getStaticPaths() {
   let lastModified = "";
   if (Astro.server) {
      const res = await fetch(`${Astro.site.origin || ""}/api/last-modified?file=src/content/docs/docs/ethical-hacking.mdx`);
...
<p class="text-sm text-slate-500 dark:text-slate-400 mb-8">
   Última actualización: {lastModified ? new Date(lastModified).toLocaleDateString("es-ES") : "N/A"}
</p>
