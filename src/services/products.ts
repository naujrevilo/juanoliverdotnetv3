...
async function getSyscomToken(): Promise<string | null> {
    ...
    const clientId = import.meta.env?.SYSCOM_CLIENT_ID || process.env.SYSCOM_CLIENT_ID;
    const clientSecret = import.meta.env?.SYSCOM_CLIENT_SECRET || process.env.SYSCOM_CLIENT_SECRET;

    if (!clientId || !clientSecret) {
        console.warn('Faltan credenciales de Syscom (SYSCOM_CLIENT_ID, SYSCOM_CLIENT_SECRET)');
 ...
 */
export async function getExternalProducts(options?: FilterOptions): Promise<PaginatedResult> {
    // 1. Verificar Cache
    ...
    try {
        ...
        if (!hasSearch && !hasCategory) {
             ...
             url.searchParams.append('categoria', SYSCOM_CATEGORIES['Videovigilancia']);
        } else {
            // Manejo de Búsqueda
        ...
        const data = await response.json();
        console.log('Respuesta Syscom recibida. Items:', Array.isArray(data) ? data.length : (data.productos?.length || 0));
        ...
        const mappedItems = items.map((item: any) => {
            ...
            return {
                ...
                createdAt: new Date(),
                source: 'external',
                externalUrl: item.link ? `https://www.syscomcolombia.com${item.link}` : undefined
            };
        });
        ...
        // Guardar en Cache
        productCache.set(cacheKey, { data: result, expiry: Date.now() + CACHE_TTL });
 ...
 */
export async function getAllProducts(options?: FilterOptions): Promise<PaginatedResult> {
