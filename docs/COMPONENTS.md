...
Barra de acciones para contenido público con URL canónica. Centraliza la codificación de enlaces para compartir y la accesibilidad de los botones.

```mermaid
flowchart LR
        Page["Página pública"] --> Props["url, title, description, actions"]
        Props --> ShareActions["ShareActions.astro"]
        ShareActions --> Networks["Redes y correo"]
        ShareActions --> Print["Imprimir/PDF"]
        Print --> BlogOnly["Solo artículos de blog"]
```

**Props:**

- `url` (string, requerido): URL canónica, normalmente `Astro.url.href`.
- `title` (string, requerido): Título incluido en los intents de compartir.
- `description` (string, opcional): Texto incluido en el correo; usa el título si no se especifica.
- `actions` (array, opcional): Acciones y orden visibles. Valores permitidos: `x`, `linkedin`, `facebook`, `whatsapp`, `mastodon`, `bluesky`, `email`, `print`.
...
<ShareActions
    ...
    actions={["x", "linkedin", "whatsapp", "email"]}
/>
```

**Criterios de uso:**

- Usar en páginas individuales de blog, recursos y proyectos, y en catálogos públicos como `/servicios`.
- Incluir `print` solo en artículos editoriales del blog; carga las imágenes diferidas antes de abrir el diálogo de impresión.
- No usar en tienda, carrito, checkout, administración, autenticación, comentarios ni endpoints API.
...
Tarjeta individual de proyecto para el portafolio. Muestra imagen, estado, categoría, tecnologías y enlaces (demo/código).

**Props:**

- `project` (Project, requerido): Entrada de la colección `projects`.
- `delay` (number, opcional): Retraso de animación en ms.

**Ejemplo:**

```astro
<ProjectCard project={project} delay={120} />
```
...
Lista interactiva de proyectos con filtros por categoría y tecnología.

**Props:**

- `projects` (array): Lista de proyectos normalizada para el cliente (slug + data).

**Características:**

- Filtros por categoría y tecnología
- Badges de categoría con color contextual
- Estado “sin resultados” con botón de limpieza
- Compatible con SSR + hidratación (`client:load`)
